...
contract("Bridge", function () {
  ...
  it("should register a foreign bridge implementation correctly", async function () {
    ...
    const vm = await signAndEncodeVM(1, 1, testGovernanceChainId, testGovernanceContract, 0, data, [testSigner1PK], 0, 0);
  ...
  it("should correctly deploy a wrapped asset for a token attestation", async function () {
    ...
    const vm = await signAndEncodeVM(0, 0, testForeignChainId, testForeignBridgeContract, 0, data, [testSigner1PK], 0, 0);
    ...
    const wrappedAddress = await initialized.methods
      .wrappedAsset("0x0001", "0x000000000000000000000000b7a2211e8165943192ad04f5dd21bedc29ff003e")
      .call();
  ...
  it("should correctly update a wrapped asset for a token attestation", async function () {
    ...
    let vm = await signAndEncodeVM(0, 0, testForeignChainId, testForeignBridgeContract, 0, data, [testSigner1PK], 0, 0);
    ...
    vm = await signAndEncodeVM(0, 0, testForeignChainId, testForeignBridgeContract, 1, data, [testSigner1PK], 0, 0);
  ...
  it("should mint bridged assets wrappers on transfer from another chain and handle fees correctly", async function () {
    ...
    const wrappedAddress = await initialized.methods
      .wrappedAsset("0x" + testBridgedAssetChain, "0x" + testBridgedAssetAddress)
      .call();
    ...
    assert.equal(accountBalanceAfter.toString(10), new BigNumber(new BigNumber(amount).minus(fee)).toString(10));
  ...
  it("should handle ETH deposits correctly", async function () {
    ...
    await initialized.methods
      .wrapAndTransferETH("10", "0x000000000000000000000000b7a2211e8165943192ad04f5dd21bedc29ff003e", fee, "234")
      .send({
        value: amount,
        from: accounts[0],
        gasLimit: 2000000,
  ...
  it("should handle ETH withdrawals and fees correctly", async function () {
    ...
    assert.equal(
      new BigNumber(new BigNumber(accountBalanceAfter).minus(accountBalanceBefore)).toString(10),
  ...
  it("should revert on transfer out of a total of > max(uint64) tokens", async function () {
    ...
    try {
      await initialized.methods
        .transferTokens(
          TokenImplementation.address,
          new BigNumber(new BigNumber(supply).minus(firstTransfer)).toString(10),
          "10",
          "0x000000000000000000000000b7a2211e8165943192ad04f5dd21bedc29ff003e",
          "0",
          "0"
